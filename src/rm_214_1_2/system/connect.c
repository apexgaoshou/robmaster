...
uint8_t Connect_TxPacket[4];				//定义发送数据包数组，数据包格式：FF 01 02 03 04 FE
uint8_t Connect_RxPacket[4];				//定义接收数据包数组
uint8_t Connect_RxFlag;					//定义接收数据包标志位
...
void Connect_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		Connect_SendByte(Array[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
void Connect_SendString(char *String)
{
	...
	{
		Connect_SendByte(String[i]);		//依次调用Serial_SendByte发送每个字节数据
	}
}
...
void Connect_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		Connect_SendByte(Number / Connect_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Serial_SendByte发送每位数字
	}
}
...
{
	Connect_SendByte(ch);			//将printf的底层重定向到自己的发送字节函数
	return ch;
}
...
void Connect_Printf(char *format, ...)
{
	...
	va_end(arg);					//结束变量arg
	Connect_SendString(String);		//串口发送字符数组（字符串）
}
...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 说    明：调用此函数后，Connect_TxPacket数组的内容将加上包头（FF）包尾（FE）后，作为数据包发送出去
  */
...
void USART3_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART3, USART_IT_RXNE) == SET)		//判断是否是USART1的接收事件触发的中断
	{
		...
		{
			Connect_RxPacket[pRxPacket] = RxData;	//将数据存入数据包数组的指定位置
			pRxPacket ++;				//数据包的位置自增
		...
		else if (RxState == 2)
		{
			...
			{
				RxState = 0;			//状态归0
				Connect_RxFlag = 1;		//接收数据包标志位置1，成功接收一个数据包
			}
		}
